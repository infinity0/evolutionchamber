...
public class EcBuildOrder extends EcState implements Serializable
{
	...
	protected int	evolvingLairs;
	public int evolvingSpires = 0;
	public int	queensBuilding=0;
	protected int	spiresInUse=0;
	protected int	evolutionChambersInUse;
	...
	{
		return ("@"+seconds()+" M:"+(int)minerals+" G:"+(int)gas + " L:"+larva + " S:"+supplyUsed+"/"+supply());
	}
	...
	public void consumeLarva(final EcEvolver e)
	{
		...
		if (!buildingLarva)
		{
			...
			addFutureAction(15,new Runnable(){
				...
				public void run()
				{
					if (e.debug) System.out.println("@"+seconds()+" Larva+1");
	...
	public double mineMinerals()
	{
		...
		for (int i = 0;i < mineralPatches();i++)
			patches[i] = 0;
		for (int i = 0;i < mineralPatches();i++)// Assign first drone
			if (drones > 0)
				{patches[i]++;drones--;}
		if (drones > 0)
		for (int i = 0;i < mineralPatches();i++)// Assign second drone
			if (drones > 0)
				{patches[i]++;drones--;}
		if (drones > 0)
		for (int i = 0;i < mineralPatches();i++)// Assign third drone
			if (drones > 0)
				{patches[i]++;drones--;}
		//Assume half the patches are close, and half are far, and the close ones have more SCVs. (perfect)
	...
	{
		if (gasExtractors == 0) return 0;
		int drones = dronesOnGas;
		int[] extractors = new int[gasExtractors]; //Assign drones/patch
		for (int i = 0;i < extractors.length;i++)
			extractors[i] = 0;
		for (int i = 0;i < extractors.length;i++)// Assign first drone
			if (drones > 0)
				{extractors[i]++;drones--;}
		for (int i = 0;i < extractors.length;i++)// Assign second drone
			if (drones > 0)
				{extractors[i]++;drones--;}
		for (int i = 0;i < extractors.length;i++)// Assign third drone
			if (drones > 0)
				{extractors[i]++;drones--;}
		//Assume half the patches are close, and half are far, and the close ones have more SCVs. (perfect)
		double gasMined = 0.0;
